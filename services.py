...
# File processing wait timeout in seconds, configurable via env
_FA_POLL_TIMEOUT = int(os.environ.get('FA_POLL_TIMEOUT', 300))
...
def download_paper(arxiv_id, force_redownload=False):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.status == 429 and attempt < max_retries - 1:
                ...
                logger.warning('arxiv API 限速 (429)，%ds 后重试（第 %d/%d 次）...', wait, attempt + 1, max_retries - 1)
                time.sleep(wait)
            else:
                logger.error('下载论文失败: %s\n%s', e, traceback.format_exc())
                return None
        except StopIteration:
            logger.error('下载论文失败: 未找到 arxiv_id=%s', arxiv_id)
            return None
    ...
    if paper is None:
        logger.error('下载论文失败: 多次重试后仍然 429，arxiv_id=%s', arxiv_id)
        return None
            ...
            for pdf_attempt in range(pdf_download_retries):
                try:
                    ...
                    if ('429' in error_str or 'retrieval incomplete' in error_str or 'ContentTooShort' in error_str) and pdf_attempt < pdf_download_retries - 1:
                        ...
                        logger.warning('PDF 下载%s，%ds 后重试（第 %d/%d 次）...', error_type, wait, pdf_attempt + 1, pdf_download_retries - 1)
                        time.sleep(wait)
                    else:
                        logger.error('PDF 下载失败: %s\n%s', e, traceback.format_exc())
                        return None

            if not pdf_downloaded:
                logger.error('PDF 下载失败: 多次重试后仍然失败，arxiv_id=%s', arxiv_id)
                return None

            is_valid, error_msg = verify_pdf(filepath)
            if not is_valid:
                logger.error('下载后验证失败: %s', error_msg)
                _backup_corrupted(filepath)
                return None
...
def _backup_corrupted(filepath):
    try:
        ...
        logger.warning('已备份损坏文件到: %s.corrupted', filepath)
    except Exception:
        pass
...
def cleanup_corrupted_files():
    ...
    for f in Path(UPLOAD_FOLDER).glob('*.corrupted'):
        try:
            ...
            logger.warning('删除损坏文件失败 %s: %s', f.name, e)
    if removed:
        logger.info('启动清理：已删除 %d 个损坏文件: %s', len(removed), ', '.join(removed))
    return removed
...
def generate_full_analysis(pdf_path, model='qwen-long'):
    ...
    poll_timeout = int(os.environ.get('FA_POLL_TIMEOUT', _FA_POLL_TIMEOUT))
    ...
    try:
        ...
        file_id = file_obj.id
        logger.info('文件已上传，file_id=%s，大小=%.1fMB，等待处理...', file_id, size_mb)

        for waited in range(0, poll_timeout, 10):
            try:
                info = client.files.retrieve(file_id)
                status = getattr(info, 'status', None)
                logger.debug('文件状态: %s（已等待 %ds）', status, waited)
                if status == 'processed':
                    break
                ...
                logger.warning('轮询文件状态失败: %s', poll_e)
            time.sleep(10)
        else:
            _safe_delete_file(client, file_id)
            return f'全文分析失败: 文件处理超时（{poll_timeout}s），PDF 可能过大或格式异常。'
